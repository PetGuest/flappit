...
const [,, out, linesJson, txt, flap, dur] = process.argv; const FPS=30;
(async()=>{
  const b=await chromium.launch(); const p=await b.newPage();
  await p.goto('http://localhost:8767/video-split-flap.html');
  const info = await p.evaluate(async(o)=>{ window.__P = FlapVideo.plan({lines:o.lines, textColor:o.txt, flapColor:o.flap, duration:o.dur||15, lowPower:false}); const P=window.__P;
    ...
    return {W:P.W,H:P.H,TOTAL:P.TOTAL, wav: btoa(s), clicks:P.clicks.length}; }, {lines:JSON.parse(linesJson), txt, flap, dur:Number(dur)||15});
